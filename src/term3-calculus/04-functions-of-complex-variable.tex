...
\begin{theorem}
    ...
    $\mathcal{P}_f$ - количество полюсов $f$ с учетом кратности в контуре $\gamma$.
...
\begin{proof}
    ...
    $\frac{f'(z)}{f(z)} = \frac{m}{z - a} + \frac{g'(z)}{g(z)}$, второе слагаемое голоморфно с окрестности $a$. Значит,
...
\begin{example}
    ...
    Возьмём $f(z) = z - \lambda$ и $g(z) = -e^{-z}$. 
    
    На отрезке $|f(z)| = |iy - \lambda| = \sqrt{y^2 + \lambda^2} \geqslant \lambda > 1$, а
    $|g(z)| = |-e^{-iy}| = 1$, значит всё выполняется.

    На полуокружности $|f(z)| = |z - \lambda| \geqslant |z| - \lambda = R - \lambda$, а 
    $|g(z)| = |-e^{-x - iy}| = e^{-x} \leqslant 1$. То есть если $R > \lambda + 1$, то $|f| > |g|$ на $\gamma$.
...
\begin{consequence}
    \begin{enumerate}
        ...
        \item {
            ...
            ноль первого порядка у $g$, а значит и ноль перовго порядка у $f$.
        }
    \end{enumerate}
\end{consequence}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item {
            ...
            $f_i(z_0) = \tilde{z_0}$ и $\arg f_i' (z_0) = \alpha$

            Здесь нужна картинка и часть доказательства.
            %TODO, картинка
            %TODO
        }
    \end{enumerate}
...
\begin{proof}
    ...
    Тогда существует $g : \bar{\mathbb{C}} \setminus \gamma \to \mathbb{D} \Rightarrow g \circ f \in H(\mathbb{C})$ и $g \circ f \subset \mathbb{D}$, то есть
...
\begin{proof}
    ...
    принимает значения $\frac{1}{a - c}$ и $\frac{1}{b - c}$.
\end{proof}

\begin{example}
    $f(z) = \tg z \neq \pm i$ - два значения может не принимать.
\end{example}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item {
            ...
            Возьмём $b = f(a)$. Тогда $f(|z - a| < r)$ - открытое множество. Более того, 
            $f(|z - a| < r) \cap f(0 < |z - z_0| < \varepsilon) = \emptyset$ из однолистности.

            $f(|z - a| < r) \cap Cl \; f(0 < |z - z_0| < \varepsilon) = \emptyset$,  противоречие.
        }
        \item {
...
\begin{consequence}
    Если функция $f$ однолистная, то $f$ линейная.
\end{consequence}
