...
\begin{proof}
    ...
    $(*) = 2R_n M_{R_n} \int_{0}^{\frac{\pi}{2}} e^{-\lambda R_n \sin t} \, dt \leqslant 
...
\begin{example}
    ...
    Жордана, где $\lambda = 1$ и $M_R = \sup_{|z| = R} \left | \frac{1}{1 + z^2} \right | \leqslant \frac{1}{R^2 - 1} $ 
    ($|1 + z^2| \geqslant |z^2| - 1 = R^2 - 1$).
...
\begin{proof}
    ...
    Параметризация $z = \varepsilon e^{it}$, где $t \in [a, b]$. Тогда $dz = \varepsilon e^{it} i \, dt$.
...
\begin{definition}
    Главное значение интеграла. $v.p. \int$

    $\int_{a}^{b} f(x) \, dx$, где $c$ - особая точка $c \in (a, b)$. 
...
\begin{example}
    ...
    $\lim_{\varepsilon} \int_{-1}^{-\varepsilon} + \int_{\varepsilon}^{1} = 0$, потому что
    фукнция нечетная.
...
\begin{example}
    ...
    Функция четная, поэтому $2I = \int_{-\infty}^{+\infty} \frac{\sin x}{x} \, dx = 
    v.p. \int_{-\infty}^{+\infty} \frac{\sin x}{x} \, dx$. ($\frac{e^{ix}}{x} = \frac{1}{x} + 1 + \ldots$).
    ...
    %TODO
    Тут нужна картинка (мы обходим ноль).
...
\begin{theorem}
    Пусть $f$ мероморфная функция в $\mathbb{C}$, $z_1, \ldots$ - полюсы.
...
\begin{proof}
    $\frac{1}{2\pi i} \int_{|z| = R_n} \frac{\zeta}{\zeta - z} \, d \zeta = \sum \res \frac{f(\zeta)}{\zeta - z} (*)$

    $\left | \frac{1}{2\pi i} \int_{|z| = R_n} \frac{\zeta}{\zeta - z} \, d \zeta \right | = R_n \cdot 
    ...
    $z \neq z_k$. $\res_{\zeta = z} \frac{f(\zeta)}{\zeta - k}$ - полюс первого порядка, значит:
    $\res_{\zeta = z} \frac{f(\zeta)}{\zeta - k} = f(z)$

    $\res_{\zeta = z_k} \frac{f(\zeta)}{\zeta - z} = \res_{\zeta = z_k} \frac{G_k (\zeta)}{\zeta - z}$. Так как разность - 
    голоморфная в точке $z_k$ функция. Такой вычет посчитаем так:
...
\begin{example}
    ...
    \begin{lemma}
        Существует $M$, такая что, $|\ctg z| \leqslant M \, \forall z \, |z| = \pi n + \frac{\pi}{2}$
    \end{lemma}
...
\begin{example}
    ...
    Знаем, что $(\ln \sin z)' = \ctg z$

    $\ctg z - \frac{1}{z} = (\ln \frac{\sin z}{z})'$.

    $\ln \frac{\sin z}{z} = \int_{0}^{z} (\ctg w - \frac{1}{w}) \, dw = \int_{0}^{z} \sum_{k=1}^{\infty} \frac{2w}{w^2 - \pi^2 k^2} \, dw = 
    \sum_{k = 1}^{\infty} \int_{0}^{z} \frac{2w}{w^2 - \pi^2 k^2}$ (можем переставлять, потому что есть
    равномерная сходимость). $\sum_{k = 1}^{\infty} \int_{0}^{z} \left ( \frac{1}{w - \pi k} + \frac{1}{w + \pi k} \right ) = 
    ...
    Тогда $\sin z = z \prod_{k = 1}^{\infty} \left ( 1- \frac{z^2}{\pi^2 k^2} \right )$
\end{example}
...
\begin{example}
    ...
    Значит:
    $\sum_{k = 1}^{\infty} \frac{1}{k^2} = -\frac{\pi}{2} \cdot \res_{z = 0} \frac{\ctg \pi z}{z^2}$.
    Такой вычет не очень приятно считать - раскладываем в ряд.

    Коэффициент перед $z^1$ в $\ctg \pi z = \frac{\cos \pi z}{\sin \pi z} = \frac{1 - \frac{\pi^2 z^2}{2} + \mathcal{O}(z^4)}{\pi z (1 - \frac{\pi^2 z^2}{6} + \mathcal{O}(z^4))} = 
    \frac{(1 - \frac{\pi^2 z^2}{2} + \mathcal{O}(z^4))(1 + \frac{\pi^2 z^2}{6} + \mathcal{O}(z^4))}{\pi z} = \frac{1}{\pi z} - \frac{1}{3} \pi z + \mathcal{O}(z^3)$.

    То есть коэффициент $-\frac{1\pi}{3}$.
