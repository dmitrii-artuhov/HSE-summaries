...
\begin{theorem}
    $P(\xi_{nk} = 1) = p_{nk} \in (0, 1)$ и $P(\xi_{nk} = 0) = 1 - p_{nk}$ и пусть $S_n = \xi_{n1} + \ldots + \xi_{nn}$ -- независимые.
...
\begin{remark}
    Для одинаково распределенных случ. величин:

    $\mathbb{D}_n^2 = n \cdot \sigma^2$, $a = \mathbb{E} \xi_1$
    
    $L(\delta, n) = \frac{1}{\left( n \sigma^2 \right)^{1 + \frac{\delta}{2}}} n \mathbb{E} | \xi_1 - a |^{2 + \delta} = C_{\delta} \cdot \frac{\mathbb{E} |\xi_1 - a|^{2 + \delta}}{n^{\frac{\delta}{2}} \sigma^{2 + \delta}}$
\end{remark}

\begin{theorem}
    \textbf{Берри-Эссенна}

    $\xi_1, \xi_2 \ldots$ -- независимые, одинаково распределённые случаные величины.

    Тогда $\left | P \left( \frac{S_n - \mathbb{E} S_n}{\sqrt{ \mathbb{D} S_n }} \leqslant x \right) - \Phi (x) \right | \leqslant C \frac{\mathbb{E} | \xi_1 - a |^3}{\sigma^3 \sqrt{n}}$
\end{theorem}

\begin{proof}
    Подставляем $\delta = 1$ в замечание выше.
\end{proof}
...
\Subsection{Большие уклонения}
 ...
 \begin{theorem}
    ...
    \overset{*}{=} \left( \frac{\mathbb{E} e^{\lambda \xi_1}}{e^{\lambda r}}  \right)^n = \exp \left( n (\ln \left( \mathbb{E} e^{\lambda \xi_1} \right) - \lambda r) \right)$
    ...
    $\varphi (\lambda) = \ln \mathbb{E} e^{\lambda \xi_1} - \lambda r \rightarrow \infty$, где $\lambda$ допустимое
 ...
 \begin{example}
    \begin{enumerate}
        ...
        \item {
            ...
            $\lambda r - \ln \frac{1}{1 - \lambda} = \lambda r + \ln (1 - \lambda)$ -- считаем производную по $\lambda$, ищем точку, где достигается максимум.
