...
\begin{example}
    ...
    Пусть $p = q = \frac{1}{2}$ --  вероятности пойти в 1ый и 2ой вход, а $S_n$ -- кол-во людей, которые пошли в первый вход.
    
    Нужно, чтобы $\begin{cases}
        n - c \leqslant S_n \leqslant c \\
        P(n - c \leqslant S_n \leqslant c) > \frac{29}{30} \text{ -- т.е. $\geq$ 29 дней из 30 гардероб не переполняется.}
    \end{cases}$
